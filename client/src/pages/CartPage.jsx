...
function CartItemCard({ item, isSyncing, onUpdateQuantity, onRemove }) {
  const imageUrl = getPublicAssetUrl(item.image);
  const itemKey = item.serverItemId || item.id || item.productId;

  return (
    <article className="surface-card p-4 sm:grid sm:grid-cols-[112px_minmax(0,1fr)] sm:gap-4">
      <Link
        to={item.slug ? `/products/${item.slug}` : '/products'}
        className="mb-4 block overflow-hidden rounded-[20px] bg-[radial-gradient(circle_at_top,_rgba(212,175,55,0.25),_transparent_35%),linear-gradient(135deg,_#fffdf8_0%,_#f5ecda_100%)] p-3 sm:mb-0"
      >
    ...
    <aside className="surface-card h-fit p-6">
      ...
      <div className="mt-6 space-y-4">
        <div className="flex items-center justify-between text-sm text-slate-600">
          ...
          <span className="font-semibold text-navy">{totalQuantity}</span>
        </div>
        <div className="flex items-center justify-between text-sm text-slate-600">
          <span>Tạm tính</span>
...
function CartPage() {
  const { cartItems, totalQuantity, totalPrice, isInitialized, isSyncing, cartError, updateQuantity, removeFromCart, clearCart } = useCart();
